...
module.exports = {
  ...
  body: (p) => {
    ...
    if (!is_up) {
      if (p.smd_cutouts) {
        throw "!! error - seeed_xiao: smd_cutouts must be used with orientation: up";
      }
      if (p.pogo_pins) {
        throw "!! error - seeed_xiao: pogo_pins must be used with orientation: up";
      }
    }
    if (p.smd_cutouts && p.pogo_pins) {
      throw "!! error - seeed_xiao: pogo_pins and smd_cutouts cannot be used at the same time";
    }
